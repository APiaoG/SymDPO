...
def save_checkpoint(model, optimizer, lr_scheduler, epoch, args):
    ...
    if args.rank == 0:
        ...
        print(f"Saving checkpoint to {args.run_name}/checkpoint_final.pt")
        torch.save(checkpoint_dict, f"{args.run_name}/checkpoint_final.pt")
        if args.report_to_wandb and args.save_checkpoints_to_wandb:
            wandb.save(f"{args.run_name}/checkpoint_final.pt")


def save_final_checkpoint(model, optimizer, lr_scheduler, args):
    """
    Save training checkpoint with model, optimizer, and lr_scheduler state.
    """
    if args.fsdp:
