...
from train_utils import filter_state_dict_to_trainable
...
class OpenFlamingoTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # import time
        # time.sleep(1000)
        # if inputs["attention_mask"] is not None and inputs["attention_mask"][:, 0].sum() != inputs["attention_mask"].shape[0] :
        #     print(inputs["input_ids"])
        #     print(inputs["attention_mask"])
        #     import time
        #     time.sleep(1000)
        outputs = model(
            vision_x=inputs["images"],
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        checkpoint_dict = {
            "epoch": self.state.global_step,
            "model_state_dict": model_state,
            # "optimizer_state_dict": optim_state,
            # "lr_scheduler_state_dict": lr_scheduler.state_dict(),
        }

        print(f"Saving checkpoint to {output_dir}/checkpoint_{self.state.global_step}.pt")
        torch.save(checkpoint_dict, f"{output_dir}/checkpoint_{self.state.global_step}.pt")
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        #     wandb.save(f"{output_dir}/checkpoint_{self.state.global_step}.pt")

        # if args.delete_previous_checkpoint:
        #     if self.state.global_step > 0:
        #         os.remove(f"{output_dir}/checkpoint_{self.state.global_step-1}.pt")


class OpenFlamingoDPOTrainer(DPOTrainer):
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        if self.train_dataset is None or not has_length(self.train_dataset):
            return None
